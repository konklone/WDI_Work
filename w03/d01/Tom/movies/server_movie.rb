...
post "/movies" do
  ...
  query += "(title, image, year) VALUES"
  query += "("
  query += "'#{parsed['Title']}', '#{parsed['Poster']}',"
  query += "'#{parsed['Year']}'"
  query += ");"
  db_conn.exec( query )
  ...
  # THE CODE BELOW WAS USED TO OPEN AND POPULATE A CSV FILE.  WE ARE NOW GOING TO CREATE A DATABASE
...
get "/movies" do
  ...
  #   output_str += "#{row['image']}, from #{row['year']} at "
  # end

  db_conn.close

# CODE for pulling data out of the CSV file
