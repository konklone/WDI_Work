...
while true
  ...
  when :n
    ...
    query_str += "(store,item,number_of_items,price,buy_date) VALUES "
  ...
  when :l
    ...
    results.each do |row|
      ...
      output_str = "Number #{row['id']}: #{old_receipt['number_of_items']} "
      output_str += "#{old_receipt['item']}, from #{old_receipt['store']} at "
      output_str += "$#{old_receipt['price']} each. (#{old_receipt['buy_date']})"
      puts output_str
    end
