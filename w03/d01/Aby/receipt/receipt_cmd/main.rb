...
while true
  ...
  when :n
    ...
    query_str += "(store, item, number_of_item, price, buy_data") VALUES " 
    query_str += "('#{new_receipt[:store]}', '#{new_receipt[:item]}', #{new_receipt[:num]}',
    #{new_receipt[:price]}, '#{new_receipt[:date]}');"
    ...
    row[:store], row[:item], row[:num], row[:price], row[:date] = line.split(",")
