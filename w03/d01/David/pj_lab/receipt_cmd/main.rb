...
while true
  ...
  when :l
    ...
    # binding.pry
    results.each do |row|
      # #parallel assignment assignes the id to line count and the rest of the string to line
      # line_count, line = line.chomp.split(":")
      # #populates old_receipt hash
      # old_receipt[:store], old_receipt[:item], old_receipt[:num], old_receipt[:price], old_receipt[:date] = line.split(",")
      output_str = "Number #{row['id']}: Number of items #{row['number_of_item']}, "
