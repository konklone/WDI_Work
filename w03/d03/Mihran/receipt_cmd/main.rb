...
while true
  ...
  when :s
    ...
    db_conn = PG.connect( dbname: FILENAME + "_db" )  #open connection to db#
    ...
    puts "Mom's sum: #{mom[0]['sum']}"
    puts "Mom's avg: #{mom_avg[0]['avg']}"
  ...
  #   result = db_conn.exec("SELECT * FROM receipts;")  #### store the receipts table in the variable result ###

  # result.each do |row| ####how many receipts fore each parent ###
  ...
  when :u
    ...
    deb_conn = PG.connect( dbname: 'receipts_db')
