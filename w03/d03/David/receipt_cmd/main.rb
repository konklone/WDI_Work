...
while true
  ...
  when :s
    ...
    result = db_conn.exec("SELECT parent FROM receipts GROUP BY parent;")
    mom = result[1]["parent"]
    dad = result[2]["parent"]
    puts "The name of the parents are #{mom} and #{dad}"

    result = db_conn.exec("SELECT SUM(number_of_item), parent FROM receipts GROUP BY parent;")
    mom_presents = result[1]["sum"]
    dad_presents = result[2]["sum"]
    puts "#{mom} bought #{mom_presents} presents and #{dad} bought #{dad_presents} presents"

    result = db_conn.exec("SELECT AVG(price), parent FROM receipts GROUP BY parent;")
    mom_cost = result[1]["avg"].to_f.round(2)
    dad_cost = result[2]["avg"].to_f.round(2)
    puts "#{mom} spent an average of $#{mom_cost} and #{dad} spent an average of $#{dad_cost}"
    ...
    mom_receipts.each do |x|
      puts "Mom bought #{x["number_of_item"]} #{x["item"]} for $#{x["price"]} at #{x["store"]} on #{x["buy_date"]}"
    end
    puts "\n"
    dad_receipts.each do |x|
      puts "Dad bought #{x["number_of_item"]} #{x["item"]} for $#{x["price"]} at #{x["store"]} on #{x["buy_date"]}"
    end
  ...
  when :u
    ...
    value = menu_gets( "Enter a new value: ", "String" )
    db_conn = PG.connect( dbname: FILENAME + "_db" )
