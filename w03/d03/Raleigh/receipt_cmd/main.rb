...
while true
  ...
  when :s
    ...
    mom = db_conn.exec("SELECT parent FROM receipts WHERE parent='Mom';")
    dad = db_conn.exec("SELECT parent FROM receipts WHERE parent='Dad';")

    # Get total presents bought
    mom_total = db_conn.exec("SELECT SUM(number_of_item) FROM receipts WHERE parent='Mom';")
    dad_total = db_conn.exec("SELECT SUM(number_of_item) FROM receipts WHERE parent='Dad';")
    total = db_conn.exec("SELECT SUM(number_of_item) FROM receipts");

    # Get average of presents
    ...
    # Get group of presents
    mom_pres = db_conn.exec("SELECT item FROM receipts WHERE parent='Mom'  ORDER BY price DESC;")
    dad_pres = db_conn.exec("SELECT item FROM receipts WHERE parent='Dad' ORDER BY price DESC;")

    # binding.pry 

    db_conn.close

    puts "Mom bought #{mom_total[0]['sum']} presents, averaging $#{mom_avg[0]['avg'].to_i.round(2)}\n" + "Presents bought:\n"
    mom_pres.each {|item| puts item.values}
    puts "\n"
    puts "Dad bought #{dad_total[0]['sum']} presents, averaging $#{dad_avg[0]['avg'].to_i.round(2)}\n" + "Presents bought:\n"
    dad_pres.each {|item| puts item.values}
