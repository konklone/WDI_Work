...
while true
  ...
  when :s
    ...
    parents.each do |parent|
      ...
      total = (db_conn.exec("SELECT COUNT(*) FROM receipts WHERE parent='#{parent}';"))[0]["count"]
      avg = (db_conn.exec("SELECT AVG(price / number_of_item) FROM receipts WHERE parent='#{parent}';"))[0]["avg"]
      presents = db_conn.exec("SELECT * FROM receipts WHERE parent='#{parent}' ORDER BY price;")


      puts "#{parent} has #{total} receipts at an average cost of $#{avg.to_f.round(2)} per present. Here are their presents:\n"
      puts " "
      presents.each do |row|
        puts row['item']
      end
