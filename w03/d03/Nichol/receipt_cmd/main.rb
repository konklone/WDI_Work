...
while true
  ...
  when :s
    ...
    # SQL IS:
    # SELECT COUNT(*) FROM receipts WHERE parent = 'Mom';
    # SELECT COUNT(*) FROM receipts WHERE parent = 'Dad';
    # SELECT AVG(price) from receipts;
    # SELECT * FROM receipts WHERE parent = 'Mom' ORDER BY price DESC;
    # SELECT * FROM receipts WHERE parent = 'Dad' ORDER BY price DESC;

    db_conn = PG.connect( dbname: FILENAME + "_db" )

    #set query, execute against database, assign variable for mom
    query = "SELECT COUNT(*) FROM receipts WHERE parent = 'Mom';"
    response = db_conn.exec(query)
    mom_receipts = response[0]["count"]

    #set query, execute against database, assign variable for dad
    query = "SELECT COUNT(*) FROM receipts WHERE parent = 'Dad';"
    response = db_conn.exec(query)
    dad_receipts = response[0]["count"]
    
    query = "SELECT AVG(price) from receipts;"
    response = db_conn.exec(query)
    avg_price = response[0]["avg"]

    query = "SELECT * FROM receipts WHERE parent = 'Mom' ORDER BY price DESC"
    response = db_conn.exec(query)
    response.each do |row|
      puts row["item"] + row["store"]
    end
